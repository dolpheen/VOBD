...
VOBD.SectorMenu.prototype.update = function(delta){
    ...
    for(var i = 0, items = this.updateItems.items;  i  < items.length; i++)
        if(items[i].update(delta).done) items.splice( i--, 1);
...
VOBD.UpdateItem.prototype.init = function(){
    this.currValue = this.object[this.key];
    this.step = ( this.value - this.currValue ) / 1000 * this.speed * 100;
    this.done = false;
}
...
VOBD.UpdateItem.prototype.update = function(delta){
    if(! this.done){
        ...
        if( dif > Math.abs(this.step)){
            this.currValue += (this.step * delta);
            if(this.step < 0 && this.currValue < this.value) this.currValue = this.value;
            if(this.step > 0 && this.currValue > this.value) this.currValue = this.value;
            this.object[this.key] = this.currValue;
...
VOBD.UpdateItems.prototype.add = function(updateItem){
    ...
    //We should repalce the IpdateUitem if it already exist in the array
