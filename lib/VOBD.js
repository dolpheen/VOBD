...
VOBD.Shader.GlowShaderA = function(viewPortWidth, viewPortHeight){
    ...
    this.uniforms = {
        ...
        "uGlowColor"  : { type: "v3", value: new THREE.Vector3(1.0, 0.1, 0.01)},
...
/**
 *  Sector is a THREE.Mesh object with custom geometry and our glow shader
 * @param innerRadius
 * @constructor
 */
...
VOBD.SectorMesh = function(parameters){
    ...
    sectorMaterial.uniforms.uColMul.value = 0.13;
...
VOBD.SectorMenu = function(parameters){
    ...
    this._oldSelectedSector = 0;
    this.meshes.rotation.z = DU.ang2Rad(53);
};

VOBD.SectorMenu.prototype.update = function(){

    //Check weather user selected other sector
    if(this.selectedSector != this._oldSelectedSector){

        if(this._oldSelectedSector > 0){
            var uniforms = this.meshes.children[this._oldSelectedSector - 1].material.uniforms;
            var updateItem = new VOBD.UpdateItem({
                object: uniforms["uAlpha"],
                key: "value",
                value: 0.27});
            var updateItem2 = new VOBD.UpdateItem({
                object: uniforms["uColMul"],
                key: "value",
                value: 0.13});
            var updateItem3 = new VOBD.UpdateItem({
        ...
        if(this.selectedSector > 0){
            var uniforms = this.meshes.children[this.selectedSector - 1].material.uniforms;
            ...
            updateItem2.update();
            updateItem3.update();
            /*this.meshes.children[this.selectedSector - 1].material.uniforms["uAlpha"].value = 0.45;
            this.meshes.children[this.selectedSector - 1].material.uniforms["uColMul"].value = 0.09;
            this.meshes.children[this.selectedSector - 1].material.uniforms["uStepMul"].value = 0.15;*/
        }
...
VOBD.UpdateItem.prototype.update = function(){
